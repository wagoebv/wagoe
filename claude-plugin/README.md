...
Ten skills. `wagoe` routes a request to the right command; the rest each own
...
| `wagoe-deploy` | Packaging and shipping a release |
...
a documented REPL alias that could not start the system for want of a JDBC
driver, and a failed production boot that wrote POSTGRES_PASSWORD to the
container log. Each skill records the behaviour that was actually observed,
