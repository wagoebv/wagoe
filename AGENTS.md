...
| **Branch protection** | `bb check:branch-protection` | Required status checks naming jobs CI never emits — a gate that can neither be satisfied nor fail. Skips when `gh` has no admin-scoped token | Yes (phantom contexts); note (test jobs not required) |
