...
Accepted — `libs/core/src/wagoe/core/validation/`.
...
`devex-validation-enabled?` are gone. `WAG_DEVEX_VALIDATION` is still registered
in `wagoe.core.config.feature-flags`; no production namespace reads it, and the
one remaining reader is a snapshot-test helper that reads the environment
variable directly.
====
