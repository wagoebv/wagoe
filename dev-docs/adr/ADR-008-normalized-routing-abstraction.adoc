...
Superseded by xref:ADR-037-reitit-routes-directly.adoc[ADR-037]. The format and
the `IRouter` port it was translated through are both gone; modules emit Reitit
route data. What remains true is the `IHttpServer` half — the server is still
pluggable, and that port has an implementation.
...
- link:../../docs/modules/architecture/pages/fc-is.adoc[Ports and Adapters Pattern]
- link:../../docs/modules/guides/pages/create-module.adoc[Guide: Add REST Endpoint]
