...
Accepted — `libs/shared-ui/`.

== Context

The Wagoe Framework supports three interaction modes as specified in the PRD: REST API, CLI, and Web Frontend. While REST API and CLI interfaces are well-established, the web frontend requires architectural decisions that align with the framework's core principles.
...
- Shared components: Complete UI component library in `wagoe.shared.ui.core`
...
**Key Decision**: Web UI is integrated directly into existing domain modules (user, user, inventory) rather than creating a separate `wagoe.web-ui` module. This approach:
...
1. **Shared Component Library (Implemented)**
   - Reusable UI components in `wagoe.shared.ui.core.components`
...
**Shared UI Component Library** - `link:https://github.com/wagoebv/wagoe/blob/main/libs/shared-ui/src/wagoe/shared/ui/core/`
...
**Static Assets:**
- ✅ **HTMX v2.0.8** - Vendored locally at `/js/htmx.min.js` (BSD 2-Clause license).
  The 2.x pin is a decision, not an accident — see
  xref:ADR-038-htmx-version-policy.adoc[ADR-038]
...
- [x] Create shared UI component library in `wagoe.shared.ui.core`
...
(ns wagoe.user.shell.web-handlers
  (:require [wagoe.user.core.ui :as user-ui]
...
;; libs/user/src/wagoe/user/core/ui.clj  
...
;; libs/user/src/wagoe/user/shell/http.clj (extend existing routes)
(defn routes []
...
- link:../reference/wagoe-prd.adoc[Product Requirements Document] - Product vision and requirements
- link:../../docs/modules/architecture/pages/fc-is.adoc[Architecture Overview] - Framework architectural principles

---

*This ADR establishes the technical foundation for module-integrated web UI development in the Wagoe Framework. Phase 1 implementation is complete with shared components and integration foundation. Phase 2 will implement complete user management web interface.*