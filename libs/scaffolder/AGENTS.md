...
id a 404, thrown as `:validation-error` / `:not-found` so the platform answers
them in its one error shape. The list takes `?limit=` (default 20, at most 100)
and `?offset=`, oldest first. Every entity's API is generated by the same code.

Every API route requires a signed-in user and answers 401 without one. The
...
- Wires it in `module_wiring.clj` and serves its CRUD API at
  ...
  `--public-api` (`:public-api true`) serves them without a signed-in user.
...
| `date` | a `YYYY-MM-DD` string that parses as a real day | `DATE` column; reads back as the same string. Adding one to a module generated before this warns when its persistence file has no `date->iso` |
