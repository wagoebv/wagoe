...
An app generated by `wagoe new` already has that `:mcp` alias in its `deps.edn`
and runs the same command.

> **Use the alias, not `-Sdeps`.** `-Sdeps` *merges* with the project's own
> `deps.edn`, which in the monorepo puts `resources/` on the classpath —
> including a `logback.xml` whose console appender writes to **stdout**, the
> stream stdio MCP reserves for JSON-RPC. Two `logback.xml` files on one
> classpath is itself a configuration warning, and any warning makes logback
> dump its entire status log there too.
>
> Measured: **88 lines of logback output preceded the first JSON-RPC
> response.** The `:mcp` alias uses `:replace-paths []` and `:replace-deps`, so
> `resources/` never joins the classpath and this library's own
> stderr-targeted `logback.xml` applies. It cannot be fixed inside the server —
> logback initialises during classpath startup, before `-main` runs.
