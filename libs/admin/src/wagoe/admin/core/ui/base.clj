...
(defn format-instant
  "Render a stored timestamp for display, in `display`'s zone and pattern.
   ...
   swallowed, so a format nobody anticipated stays visible."
  [value display]
  (if-let [instant (tc/string->instant value)]
    (.format (DateTimeFormatter/ofPattern
              (or (:date-time-format display) default-instant-format))
             (.atZone instant (display-zone display)))
    (str value)))
...
(defn format-date
  ...
  (if-let [instant (tc/string->instant value)]
    (.format (DateTimeFormatter/ofPattern
              (or (:date-format display) default-date-format))
             (.atZone instant (display-zone display)))
    (str value)))
