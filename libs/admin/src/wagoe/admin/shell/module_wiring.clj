...
(defn ig-config
  ...
  ;; `:date-time-format` — which live under `:wagoe/settings` — were out of
  ;; reach of every admin handler. Nothing read them anywhere and the list
  ;; table rendered raw database timestamps (BOU-382). Copied in rather than
  ;; handing the whole application config to the routes, which would let any
  ;; handler reach anything.
