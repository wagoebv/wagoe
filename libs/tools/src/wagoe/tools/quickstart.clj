...
(def ^:private sample-config-snippet
  ...
  (str "  ;; Tasks module (generated by quickstart)\n"
       "  :wagoe/tasks\n"
       "  {:enabled? true\n"
       "   :base-path \"/api/tasks\"}"))

(defn inject-sample-module-config
  "Add the sample tasks module to every profile's config.edn, with the rules
   `bb scaffold integrate` uses — the fallback when that step fails (BOU-529)."
  ([] (inject-sample-module-config (System/getProperty "user.dir")))
  ([root]
   (let [results (integrate/write-config! root ":wagoe/tasks" sample-config-snippet {})]
     (and (empty? (integrate/blocking results))
          (boolean (some #{:written :already-present} (map second results)))))))
...
(defn- print-finish
  ...
  (println "    2. Eval " (bold "(go)") " at the prompt — or connect your editor")
  (println "       (or the Wagoe MCP server) to port 7888 and eval it there")
