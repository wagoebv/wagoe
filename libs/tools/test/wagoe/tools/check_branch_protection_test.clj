...
(deftest ^:unit a-parked-job-is-not-required
  ...
  ;; that can never be satisfied, which sits pending and reads like passing.
  ;; Parking is still reported, because it is how a job leaves the merge gate.
...
(deftest ^:unit the-repository-is-currently-consistent
  ...
  (let [{:keys [missing summary-name disabled]} (sut/summary-covers (sut/ci-workflow))]
    ...
    (testing "and no job is parked"
      ;; e2e was, for two milestones, on the grounds that it needed a live
      ;; server CI did not start — while its own steps ran `bb e2e`, which
      ;; starts one. 52 tests over login, registration, MFA, sessions and admin
      ;; CRUD sat outside the merge gate on a reason that had stopped being true
      ;; (BOU-297).
      ;;
      ;; Parking is still available and still legitimate. What it may not be is
      ;; quiet: an entry here is the decision to let a job fail without blocking
      ;; a merge, and it should be argued for in the same breath.
      (is (empty? disabled)
  ...
  (testing "and the e2e suite in particular is required"
    ...
    ;; `clojure -M:test` says nothing about.
    (let [{:keys [missing disabled]} (sut/summary-covers (sut/ci-workflow))]
      (is (not (some #{"e2e"} missing)))
      (is (not (some #{"e2e"} disabled)))))
