...
(defn- instant->db [^Instant i] (when i (Timestamp/from i)))

(defn- db->instant
  "What each driver hands back for a TIMESTAMP WITH TIME ZONE column, as an
   Instant. SQLite has no such type and returns epoch millis — as text in a
   table created before BOU-502, where the column is still TEXT."
  [v]
  (condp instance? v
    Instant        v
    OffsetDateTime (.toInstant ^OffsetDateTime v)
    Date           (.toInstant ^Date v)
    Number         (Instant/ofEpochMilli (long v))
    String         (if-let [ms (parse-long v)]
...
(defn- instance->db
  ...
  {:id           (uuid->str (:id instance))
   ...
   :created_at   (instant->db (:created-at instance))
   :updated_at   (instant->db (:updated-at instance))
...
(defn- db->instance
  ...
  (when row
    (cond-> {:id            (str->uuid (:id row))
             ...
             :created-at    (db->instant (:created_at row))
             :updated-at    (db->instant (:updated_at row))}
      (:metadata row)
...
(defn- entry->db
  ...
  {:id           (uuid->str (:id entry))
   ...
   :occurred_at  (instant->db (:occurred-at entry))})
...
(defn- db->entry
  ...
  (when row
    (cond-> {:id          (str->uuid (:id row))
             ...
             :occurred-at (db->instant (:occurred_at row))}
...
(defrecord WorkflowStore [datasource]
  ...
  (update-instance-state! [_ instance-id new-state]
    ...
    (let [now (instant->db (Instant/now))]
      (jdbc/execute-one! datasource
