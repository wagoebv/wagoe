...
#
# The two environment cells the matrix promised and never had: a machine with a
# JDK too old to use, and a machine that cannot reach the network. Both need a
# container of their own — the smoke and adversarial suites each install once
# into a container they then reuse, and neither shape can host these.
#
# Both assert on the *message*. A first-run tool that fails is survivable; one
# that fails without saying what to do next is what makes someone close the tab.
#
# Run:
...
echo "[1/2] a JDK older than 21 is not silently accepted"
if docker run --rm \
...
echo "[2/2] no network fails with a message, not a stack trace"
...
echo
if [ "$FAILED" -eq 0 ]; then
  echo "Preconditions: both cases passed."
else
