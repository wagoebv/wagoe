...
| Release candidate. The `1.0.0` surface is frozen: a breaking change lands
  ...
  then in an RC, never in `1.0.0` itself. Some have — see
...
* *One has, in `1.0.0-rc-2`:* a `:wagoe/*` key in `:active` that nothing
  ...
  blocks in `:wagoe/config-keys`.
* *One more, in `1.0.0-rc-3`:* `bb scaffold generate --cli` is gone. It named an
  interface the scaffolder has never had a generator for, so no value of the
  flag ever produced one; only a script that passes it is affected.
* *Six in `1.0.0-rc-4`:* workflow's timestamp columns carry a zone and need
  `migrate up` with replicas stopped; `:wagoe/logging :level` now sets
  Logback's levels; `register-user` throws `:validation-error` for a weak
  password; and the scaffolder guards generated APIs, makes `date` a `DATE`
  and refuses unknown field modifiers. Each, with its migration, is under
  `### Breaking` in `CHANGELOG.md`. Eight additions to the three frozen at
  `rc-1`, so the list is eleven.
...
in a `1.x` release. What `2.0` and the `1.x` releases before it will build is on
the xref:roadmap.adoc#_after_1_0_0[roadmap].
...
* xref:roadmap.adoc[Roadmap] — what is left before `1.0.0`, and what comes after
