= Monorepo Structure
:description: How the Wagoe repository is organized and how libraries relate.

Wagoe is a monorepo of 31 independently publishable libraries under `libs/`.
...
├── libs/                  ← 31 independently publishable libraries
│   ├── core/
│   ├── observability/
│   ├── platform/
│   ├── user/
...
Example applications live under `examples/`, and CI runs both on every commit:

* `examples/shop` — a generated application: `wagoe new` plus one scaffolded
  module, committed as generator output and re-verified by
  `bb example:regen --check`
* `examples/todo` — functional core / imperative shell with no framework at all

https://github.com/wagoebv/wagoe-examples[wagoe-examples] holds community
examples. They are not versioned against a release and not built here.
...
[source,bash]
----
export WAG_ENV=dev           # loads resources/conf/dev/config.edn
