...
Several libraries already ship both adapters, and they select one the same
way: `:provider :memory | :redis | :db`. The pre-1.0 spellings — `:in-memory`,
`:redis-streams`, `:database` — still work and warn (BOU-436):
