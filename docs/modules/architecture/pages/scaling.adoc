...
| Opt-in: pass a `:cache` component and the breaker keeps its state there, so
  ...
  Trips on consecutive `:rpc/unavailable` and `:rpc/timeout` — failures where
  the call did not reach the service. A remote error does not count: the
  service answered, so it is up. Note that `:rpc/timeout` trips the breaker
