...
In the test profile `service user` runs 19 of the application's 33 components.
...
. ✅ *Generic remote-port adapter + RPC envelope* — done (BOU-90), less the circuit breaker (BOU-285). `clj-http` client implementing a module protocol over transit, context propagation, typed errors, retry bounded to calls that never executed.
