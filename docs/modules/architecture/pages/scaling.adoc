...
[source,clojure]
----
;; In the process that owns payments — serve the protocol, in shell/http.clj
(defn routes [provider service-key]
  (merge (rpc-server/rpc-routes pay-ports/IPaymentProvider provider
...
Use `rpc-routes` rather than writing the route map by hand. It returns a `:web`
route, and the slot matters: the router rewrites `:api` paths with the version
prefix, so a handler mounted there is served at `/api/v1/rpc` while a client on
the default `:path` posts to `/rpc` and gets a 404 — a failure that reads as
the service being down rather than being somewhere else. A service that does
mount it under `:api`, or anywhere else, tells the client with `:path`.
