...
:description: Conventions, safe patterns, and recipes for Wagoe database migrations.
:toc:

Wagoe uses https://github.com/yogthos/migratus[Migratus] for SQL-based, versioned migrations. This guide covers the naming convention, safe patterns, and common recipes.
...
Libraries contribute migrations by publishing a `wagoe/migration-paths/<library>.edn` manifest on the classpath. The file name must be unique to the library: an uberjar keeps only one copy of a shared name.

[source,clojure]
----
;; libs/geo/resources/wagoe/migration-paths/geo.edn
...
When using `wagoe-tenant` (schema-per-tenant), migrations run in two scopes:
