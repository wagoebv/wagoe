...
=== Deployment (framework repository only)

These publish the Wagoe libraries themselves and exist only in the framework
repository — a project created by `wagoe new` has no `bb deploy` task.
...
`--edn` prints the results as data instead of for a person; that is what
`wagoe doctor` reads so it can end with a single next action.
