...
Development-only library providing x-ray vision into running Wagoe applications. Zero production overhead — loaded only via the `:dev` and `:repl-clj` aliases.

NOTE: Published to Clojars as `com.wagoe/wagoe-devtools`, in lockstep with the
rest of the suite. Dev-only describes when you use it — inside this monorepo it
is wired in as a local dependency via `libs/devtools`.
