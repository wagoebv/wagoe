...
The five above — plus `(status)`, `(fix!)` and `(commands)` — are in the
`dev/user.clj` of every project created by `wagoe new`. The rest of this page
describes helpers wired in this monorepo's own `dev/repl/user.clj`; in your
project, call them through their namespaces:

[source,clojure]
----
;; Schema exploration
(require (quote [wagoe.devtools.core.schema-tools :as schema-tools]))
(schema-tools/schema-tree :user/CreateUser)
----
...
[source,bash]
----
clojure -M:test:test/http :devtools
----
