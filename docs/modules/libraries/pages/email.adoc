...
| Optional integration with `wagoe-jobs` for queued sending
|===

== Transport layer

`wagoe-email` delegates raw SMTP transport to `wagoe-external`:

[source]
----
wagoe-email (application layer)  →  wagoe-external (transport layer)
